...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "Designate a target user as a thief using custom cartoon template",
    category: "fun",
    guide: {
  ...
  onStart: async function ({ message, event, api, usersData }) {
    try {
      ...
      const templateUrl = "https://i.imgur.com/LmmDNr6.jpeg";
      ...
      // Render base cartoon thief template
      ctx.drawImage(templateImg, 0, 0, width, height);

      // Position target face over the thief head area
      const avatarRadius = width * 0.17;
      const avatarX = width * 0.40;
      const avatarY = height * 0.20;
      ...
      const outputPath = path.join(cacheDir, `chor_${targetID}_${Date.now()}.png`);
      ...
      out.on("finish", async () => {
        try {
          const replyText = `Look who got caught stealing! ${targetName} is the thief! 🕵️‍♂️💰`;
          ...
          console.error("Chor delivery pipeline error:", deliveryErr);
        }
      });

    } catch (error) {
      console.error("Chor execution runtime failure:", error);
      return message.reply("❌ An error occurred while generating the thief image.");
    }
  }
};

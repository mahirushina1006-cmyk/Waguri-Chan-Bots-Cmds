...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "Kiss a user with custom romantic couple template and text mention",
    category: "fun",
    guide: {
  ...
  onStart: async function ({ message, event, api, usersData }) {
    try {
      ...
      if (Object.keys(event.mentions).length > 0) {
        ...
        return message.reply("⚠️ Please mention someone or reply to a message to execute a kiss.");
      }

      const senderID = event.senderID;
      api.setMessageReaction("💋", event.messageID);

      // Fetch user names for text mention formatting
      const senderName = await usersData.getName(senderID) || "User";
      const targetName = await usersData.getName(targetID) || "User";

      const templateUrl = "https://i.imgur.com/BGRCK1a.jpeg";

      // Robust buffer fetcher with browser User-Agent header
      ...
      // Resilient avatar fetching with fallback endpoints
      ...
      // Download required assets concurrently
      ...
      const outputPath = path.join(cacheDir, `kiss3_${senderID}_${targetID}_${Date.now()}.png`);
      ...
      out.on("finish", async () => {
        try {
          const replyText = `${senderName} kissed ${targetName}! 💋❤️`;
          ...
          console.error("Kiss3 delivery pipeline error:", deliveryErr);
        }
      });

    } catch (error) {
      console.error("Kiss3 execution runtime failure:", error);
      return message.reply("❌ An error occurred while generating the kiss image.");
    }
  }
};

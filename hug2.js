...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: "Special anime hug command with custom template and text mention",
    category: "fun",
    guide: {
  ...
  onStart: async function ({ message, event, api, usersData }) {
    try {
      ...
      const templateUrl = "https://i.imgur.com/fKQohDK.jpeg";
      ...
      // 1. Position Female Character Face (Top / Hugger - Target User)
      const targetRadius = width * 0.14;
      ...
      // 2. Position Male Character Face (Bottom / Hugged - Sender)
      const senderRadius = width * 0.13;
      const senderX = width * 0.42;
      const senderY = height * 0.53;
      ...
      const outputPath = path.join(cacheDir, `hug2_${senderID}_${targetID}_${Date.now()}.png`);
      ...
      out.on("finish", async () => {
        try {
          ...
          console.error("Hug2 delivery pipeline error:", deliveryErr);
        }
      });

    } catch (error) {
      console.error("Hug2 execution runtime failure:", error);
